...
void setup()
{
  ...
  Serial.begin(115200);  //Original: 921600
  delay(10);
  ...
  while (WiFi.status() != WL_CONNECTED)
  {
    ...
    while (WiFi.status() != WL_CONNECTED)
    {
      ...
      if (retrys++ > 10 && WiFi.status() != WL_CONNECTED)
      {
        ...
        DisplayMessage(locale.COMMON.RESTART_SYSTEM); //modifica mia
        delay(5000);   //modifica mia
        ESP.restart();
      }
    }
...
else
{
  ...
  else if (statusDisplay)
  {
    ...
    if (volumio.State.status != "stop")
      if ((Now - LastTime) < 12000 && volumio.State.status != "stop")
      {
